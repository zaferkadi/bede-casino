...
const slips = (state={items:[], isPushing: false, receipts:[]}, action) => {
	switch(action.type) {
		case types.ADD_BET:
			Object.assign(action.bet, {stake:0});
		...
		case types.REMOVE_BET:
	      ...
	      //return state.slice(0, removeIndex).concat(state.slice(removeIndex+1));
	      	return Object.assign({},state,{items: [...state.items.slice(0, removeIndex),...state.items.slice(removeIndex+1)]});

	     case types.PUSHING_BETS: 
	     	return Object.assign({},state,{isPushing:action.isPushing});	
